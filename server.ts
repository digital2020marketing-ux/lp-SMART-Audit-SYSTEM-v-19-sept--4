...
const ACCESS_TOKEN =
  ...
  'EAFzGloW5T2cBSZAzLpKCAeLVlwSGZAgoo0H47IsZAR3eFdReYZAvJioGMoNgboStInAxZBYErDIRpHVLuZAP9h6qJ7uyUs6Fqs56TNGGCMUNa6GAkxZCZCym3QYIg10LwllCHhRhh3qjlFR3CbGOeTtPYjZAbOEvhriG0gDFQHQxdAbZC7ymHJPhZCub5ZA7gx2U8AZDZD';

async function startServer() {
  const app = express();
  const PORT = 3000;

  // Enable Gzip/Brotli response compression for ultra-fast asset transfers
  app.use(compression());
  ...
  app.use(
    express.static(path.join(process.cwd(), 'public'), {
      ...
      setHeaders: (res, filePath) => {
        ...
        } else if (filePath.match(/\.(webp|jpg|png|svg|ico|css|js|woff2)$/)) {
